...
sha256, ставит, прописывает `IFACE_WAN` и запускает сервис — **но только если
`zapret2 check` прошёл без ошибок**. Если что-то не так, он не трогает firewall
и печатает, что чинить.
...
`postinst` сразу прогоняет `zapret2 check` и печатает, чего не хватает.
...
По умолчанию в конфиге стоит **`MODE_FILTER=none`**, и это означает, что списка
